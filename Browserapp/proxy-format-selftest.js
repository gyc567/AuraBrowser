...
assert.strictEqual(
  context.normalizeProxy('192.0.2.10:6099:user:pass', 'socks5'),
  'socks5://user:pass@192.0.2.10:6099'
);
assert.strictEqual(
  context.normalizeProxy('192.0.2.10:6099:user:pass', 'http'),
  'http://user:pass@192.0.2.10:6099'
);
assert.strictEqual(
  context.normalizeProxy('192.0.2.10:6099:user:pass', 'https'),
  'https://user:pass@192.0.2.10:6099'
);
assert.strictEqual(
  context.normalizeProxy('http://user:pass@192.0.2.10:6099', 'socks5'),
  'http://user:pass@192.0.2.10:6099'
);
assert.strictEqual(
  context.normalizeProxy('socks5s://user:pass@192.0.2.10:6099', 'http'),
  'socks5://user:pass@192.0.2.10:6099'
);
...
assert.strictEqual(
  context.proxyAuthActionForUpdate(existing.raw, 'socks5://old-proxy.test:1080', 'proxy'),
  'clear'
);
assert.strictEqual(
  context.proxyAuthActionForUpdate(existing.raw, 'socks5://new-proxy.test:1080', 'proxy'),
  null
);
...
console.log(
  'PROXY_FORMAT_SELFTEST_OK bare_socks5=1 bare_http=1 bare_https=1 explicit=1 alias=1 encoding=1 merge=1 redacted_restore=1 explicit_clear=1 ui_clear=1 raw_priority=1 legacy=1'
);
