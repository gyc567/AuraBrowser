...
function run(file, args) {
  return new Promise((resolve, reject) => {
    ...
    child.once('exit', (code) =>
      code === 0 ? resolve(stdout) : reject(new Error(`driver exited ${code}: ${stdout}\n${stderr}`))
    );
  });
}
...
async function main() {
  ...
  try {
    ...
    if (!sync.nativeInputMirror || sync.nativeInputMirror.exitCode !== null)
      throw new Error(
        'input bridge not running: ' + JSON.stringify({ exitCode: sync.nativeInputMirror?.exitCode, events })
      );
    ...
    for (const text of expected) {
      outputs.push(
        (
          await run(path.join(__dirname, 'native-omnibox-driver.exe'), [
            String(master.pid),
            text,
            ...sessions.slice(1).map((session) => String(session.pid)),
          ])
        )
          .trim()
          .split(/\r?\n/)
