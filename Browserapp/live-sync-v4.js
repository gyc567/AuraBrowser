  ...
  command(method, params = {}) {
    return new Promise((resolve, reject) => {
      if (!this.socket || this.socket.readyState !== WebSocket.OPEN)
        return reject(new Error('CDP is not connected'));
...
function normalTabs(values) {
  return values.filter((tab) => !/^(chrome|edge|devtools|chrome-extension|edge-extension):/i.test(tab.url));
}
...
class LiveSyncController {
  constructor(engine, emit) {
    ...
    this.timer = null;
    this.forwardQueue = [];
    this.forwardQueueRunning = false;
    this.coalescedForwards = new Map();
    this.forwardStats = { coalesced: 0, dropped: 0, processed: 0, lastLatencyMs: 0 };
    this.lastHealthEmitAt = 0;
    this.lastWatchErrorAt = 0;
    this.refreshInFlight = false;
    this.refreshTask = null;
    this.refreshEpoch = 0;
    this.skippedRefreshes = 0;
    // Adaptive sync poll: active input stays fast; idle stretches to save CPU/CDP load.
    this.lastActivityAt = 0;
    this.refreshIntervalMs = 700;
    this.tickCount = 0;
  }
  ...
  async start(ids) {
    ...
    this.emit({
      ...
      slaves: this.slaves.map((item) => item.id),
  ...
  async runRefreshTick(refreshGeneration = null, refreshEpoch = this.refreshEpoch) {
    ...
    if (
      ...
      this.getRefreshGeneration &&
      refreshGeneration !== this.getRefreshGeneration()
    )
      return;
    if (!this.master || this.refreshInFlight) {
      if (this.refreshInFlight) this.skippedRefreshes += 1;
      return;
    }
  ...
  handleWatchError(error) {
    ...
    if (/ECONNREFUSED|connection refused|fetch failed|CDP connection closed/i.test(message)) {
      const master = this.master?.id || null;
      const slaves = this.slaves.map((item) => item.id);
      this.stop();
      this.emit({
    ...
    const now = Date.now();
    if (now - this.lastWatchErrorAt < 5000) return;
    this.lastWatchErrorAt = now;
  ...
  stop() {
    ...
    this.coalescedForwards.clear();
    this.forwardQueueRunning = false;
    if (this.timer) clearTimeout(this.timer);
    this.timer = null;
    this.tickCount = 0;
    for (const value of this.connections.values()) value.connection.close();
  ...
  async refreshMasterTabs() {
    if (!this.master) return;
    const tabs = normalTabs(await cdp.tabs(this.master.item.port));
  ...
  async pollScroll(value) {
    ...
    const position = result.result?.value;
    if (!position) return;
    if (position.x !== value.scroll.x || position.y !== value.scroll.y) {
      value.scroll = position;
      this.enqueueForward(value.tab.id, { type: 'scroll', x: position.x, y: position.y });
    }
  }
  ...
  forwardKey(tabId, payload, action = 'forward') {
    ...
    if (payload.type === 'input')
      return `input:${tabId}:${payload.selector || payload.name || payload.placeholder || ''}`;
  ...
  enqueueForward(tabId, payload, action = 'forward') {
    this.markActivity();
    const key = this.forwardKey(tabId, payload, action);
    const now = Date.now();
    if (key && this.coalescedForwards.has(key)) {
      ...
      if (payload.type === 'wheel')
        entry.payload = {
          ...payload,
          deltaX: (Number(entry.payload.deltaX) || 0) + (Number(payload.deltaX) || 0),
          deltaY: (Number(entry.payload.deltaY) || 0) + (Number(payload.deltaY) || 0),
        };
      else entry.payload = payload;
      entry.queuedAt = now;
      this.forwardStats.coalesced += 1;
      this.emitForwardHealth();
      return;
    }
    if (key && this.forwardQueue.length >= 256) {
      const index = this.forwardQueue.findIndex((entry) => entry.key);
      if (index >= 0) {
        const [removed] = this.forwardQueue.splice(index, 1);
        this.coalescedForwards.delete(removed.key);
        this.forwardStats.dropped += 1;
      }
    }
    const entry = { tabId, payload, action, key, queuedAt: now };
    this.forwardQueue.push(entry);
    if (key) this.coalescedForwards.set(key, entry);
    this.drainForwardQueue();
    this.emitForwardHealth();
  }

  async drainForwardQueue() {
    if (this.forwardQueueRunning) return;
    this.forwardQueueRunning = true;
    try {
      while (this.master && this.forwardQueue.length) {
        const entry = this.forwardQueue.shift();
        if (entry.key) this.coalescedForwards.delete(entry.key);
        const started = Date.now();
        try {
          if (entry.action === 'navigate') await this.navigateSlaves(entry.tabId, entry.payload.url);
          else await this.forward(entry.tabId, entry.payload);
        } catch (error) {
          this.emit({ type: 'sync-error', action: 'forward', message: error.message });
        }
        this.forwardStats.processed += 1;
        this.forwardStats.lastLatencyMs = Date.now() - started;
        this.emitForwardHealth();
      }
    } finally {
      this.forwardQueueRunning = false;
      this.emitForwardHealth(true);
    }
  }

  emitForwardHealth(force = false) {
    const now = Date.now();
    if (!force && now - this.lastHealthEmitAt < 1000) return;
    this.lastHealthEmitAt = now;
    this.emit({
      type: 'sync-health',
      queueDepth: this.forwardQueue.length,
      skippedRefreshes: this.skippedRefreshes,
      ...this.forwardStats,
    });
  }
  ...
  async handle(tabId, event) {
    ...
    if (event.method === 'Page.frameNavigated' && !event.params?.frame?.parentId) {
      ...
      if (url && !/^(chrome|edge|devtools|chrome-extension|edge-extension):/i.test(url))
        this.enqueueForward(tabId, { url }, 'navigate');
    }
  }

  masterIndex(tabId) {
    const index = this.masterTabs.findIndex((tab) => tab.id === tabId);
    return Math.max(0, index);
  }

  async slaveTab(slave, tabId) {
    const index = this.masterIndex(tabId);
    const masterTab = this.masterTabs[index];
    let tabs = normalTabs(await cdp.tabs(slave.port));
    if (masterTab) {
      const sameUrl = tabs.find((tab) => tab.url === masterTab.url);
      if (sameUrl) return sameUrl;
    }
    while (tabs.length <= index) {
      await cdp.newTab(slave.port, masterTab?.url || 'about:blank');
      tabs = normalTabs(await cdp.tabs(slave.port));
    }
    return tabs[index] || tabs[0] || null;
  }

  async eachSlave(tabId, action) {
    await Promise.all(
      this.slaves.map(async (slave) => {
        const tab = await this.slaveTab(slave, tabId);
  ...
  urlsMatch(a, b) {
    const x = this.urlKey(a);
    const y = this.urlKey(b);
    if (!x || !y) return false;
    if (x === y) return true;
    const blank = (v) =>
      v === 'about:blank' ||
      v === 'chrome://newtab' ||
      v === 'chrome://new-tab-page' ||
      v === 'edge://newtab';
    return blank(x) && blank(y);
  }

  async navigateSlaves(tabId, url) {
    await this.eachSlave(tabId, (tab) =>
      this.urlsMatch(tab.url, url)
        ? Promise.resolve()
  ...
  async mappedMousePoint(tab, payload, focus = false) {
    ...
    const fallbackX = Number(payload.x) || 0;
    const fallbackY = Number(payload.y) || 0;
    const rx = Math.max(0, Math.min(1, Number.isFinite(Number(payload.rx)) ? Number(payload.rx) : 0.5));
    const ry = Math.max(0, Math.min(1, Number.isFinite(Number(payload.ry)) ? Number(payload.ry) : 0.5));
    const tag = JSON.stringify(String(payload.tag || '').toLowerCase());
    const role = JSON.stringify(String(payload.role || ''));
    const aria = JSON.stringify(String(payload.ariaLabel || ''));
    const text = JSON.stringify(
      String(payload.text || '')
        .trim()
        .slice(0, 200)
    );
    const elementType = JSON.stringify(String(payload.elementType || ''));
    ...
    const result = await cdp
      .call(tab.webSocketDebuggerUrl, 'Runtime.evaluate', { expression, returnByValue: true })
      .catch(() => null);
    const value = result?.result?.value;
    return value && Number.isFinite(value.x) && Number.isFinite(value.y)
      ? value
      : { x: fallbackX, y: fallbackY, found: false };
  }
  ...
  async forward(tabId, payload) {
    ...
    if (payload.type === 'click') {
      await this.eachSlave(tabId, async (tab) => {
        const point = await this.mappedMousePoint(tab, payload, true);
        const button = ['left', 'middle', 'right'][payload.button] || 'left';
        await cdp.call(tab.webSocketDebuggerUrl, 'Input.dispatchMouseEvent', {
          type: 'mousePressed',
          x: point.x,
          y: point.y,
          button,
      ...
      const type = types[payload.phase];
      if (!type) return;
      await this.eachSlave(tabId, async (tab) => {
        const point = await this.mappedMousePoint(tab, payload, payload.phase === 'down');
        await cdp.call(tab.webSocketDebuggerUrl, 'Input.dispatchMouseEvent', {
          type,
          x: point.x,
          y: point.y,
          ...
          clickCount: payload.phase === 'move' ? 0 : 1,
        });
      });
    } else if (payload.type === 'wheel') {
      if (payload.ctrl) return;
      await this.eachSlave(tabId, async (tab) => {
        const point = await this.mappedMousePoint(tab, payload, false);
        await cdp.call(tab.webSocketDebuggerUrl, 'Input.dispatchMouseEvent', {
          type: 'mouseWheel',
          x: point.x,
          y: point.y,
      ...
      await this.eachSlave(tabId, (tab) =>
        cdp.call(tab.webSocketDebuggerUrl, 'Runtime.evaluate', { expression })
      );
    } else if (payload.type === 'focus' || payload.type === 'beforeinput') {
      const selector = JSON.stringify(String(payload.selector || ''));
      const start = Number.isInteger(payload.start) ? payload.start : null;
      const end = Number.isInteger(payload.end) ? payload.end : null;
      const x = Number(payload.x) || 0;
      const y = Number(payload.y) || 0;
      const tag = JSON.stringify(String(payload.tag || ''));
      const type = JSON.stringify(String(payload.elementType || ''));
      ...
      await this.eachSlave(tabId, async (tab) => {
        ...
        if (result.result?.value || !x || !y) return;
        await cdp.call(tab.webSocketDebuggerUrl, 'Input.dispatchMouseEvent', {
          type: 'mousePressed',
          x,
          y,
      ...
      const name = JSON.stringify(String(payload.name || ''));
      const placeholder = JSON.stringify(String(payload.placeholder || ''));
      const inputType = JSON.stringify(String(payload.inputType || 'insertText'));
      const data = payload.data == null ? 'null' : JSON.stringify(String(payload.data));
      ...
      await this.eachSlave(tabId, async (tab) => {
        ...
        await cdp.call(tab.webSocketDebuggerUrl, 'Input.dispatchKeyEvent', {
          type: 'keyDown',
          key: 'a',
          code: 'KeyA',
        ...
        await cdp.call(tab.webSocketDebuggerUrl, 'Input.insertText', { text: JSON.parse(value) });
      });
      ...
      let modifiers = 0;
      if (payload.alt) modifiers |= 1;
      if (payload.ctrl) modifiers |= 2;
      if (payload.meta) modifiers |= 4;
      if (payload.shift) modifiers |= 8;
      const printable =
        String(payload.key || '').length === 1 && !payload.ctrl && !payload.alt && !payload.meta;
      if (payload.editable && (printable || ['backspace', 'delete'].includes(keyName))) return;
      const params = {
        type: payload.phase === 'up' ? 'keyUp' : 'keyDown',
        text: payload.phase === 'down' && printable ? String(payload.key) : '',
        unmodifiedText: payload.phase === 'down' && printable ? String(payload.key) : '',
      ...
      await this.eachSlave(tabId, (tab) =>
        cdp.call(tab.webSocketDebuggerUrl, 'Input.dispatchKeyEvent', params)
      );
    }
  }
}
