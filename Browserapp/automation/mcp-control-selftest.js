  ...
  const profiles = new Map([
    [
      'fp1',
      {
        ...
        number: 1,
        proxy: 'Direct',
        privacy: { fingerprint: { os: 'Windows' } },
      },
    ],
  ]);
...
async function main() {
  ...
  const syncBridge = new WindowSyncBridge({
    getLiveSync: () => null,
    beginSync: async (ids) => {
      syncState.active = Boolean(ids?.length);
  ...
  const request = (method, path, body, apiKey = 'mcp-selftest-key') =>
  ...
  record(
    'local-api create fingerprint profile',
    createResult.status === 200 && createResult.body.code === 0 && createdProfile?.id === 'mcp-fp'
  );
  record(
    'local-api create maps snake_case fields',
    createdProfile?.startUrl === 'https://example.com' &&
      createdProfile?.userAgent === 'Mozilla/5.0 MCP' &&
      createdProfile?.width === 1440 &&
      createdProfile?.height === 900 &&
      createdProfile?.note === 'created via mcp selftest'
  );
  record(
    'local-api create merges top-level fingerprint into privacy',
    createdProfile?.privacy?.fingerprint?.os === 'Linux' &&
      createdProfile?.privacy?.fingerprint?.hardwareConcurrency === 8 &&
      createdProfile?.privacy?.timezone === 'Asia/Shanghai'
  );
  ...
  record(
    'local-api update profile',
    updateResult.status === 200 &&
      updateResult.body.code === 0 &&
      updatedProfile?.name === 'MCP Fingerprint Updated' &&
      updatedProfile?.privacy?.fingerprint?.os === 'macOS' &&
      updatedProfile?.privacy?.fingerprint?.canvasId === 4242
  );
  record(
    'local-api update maps snake_case fields',
    updatedProfile?.startUrl === 'https://openai.com' &&
      updatedProfile?.userAgent === 'Mozilla/5.0 Updated' &&
      updatedProfile?.width === 2560 &&
      updatedProfile?.height === 1440 &&
      updatedProfile?.privacy?.timezone === 'America/New_York' &&
      updatedProfile?.privacy?.fingerprint?.webglRenderer === 'ANGLE (MCP GPU)'
  );

  const duplicateResult = await request('POST', '/api/v2/browser-profile/duplicate', {
    source_profile_id: 'mcp-fp',
    name: 'MCP Fingerprint Copy',
  });
  record(
    'local-api duplicate profile without exit data',
    duplicateResult.status === 200 &&
      duplicateResult.body.code === 0 &&
      duplicateResult.body.data.profile?.id !== 'mcp-fp' &&
      duplicateResult.body.data.profile?.exitNetwork === undefined
  );

  const checkResult = await request('POST', '/api/proxy/check-profile', { profile_id: 'mcp-fp' });
  record(
    'local-api check profile proxy persists',
    checkResult.status === 200 &&
      checkResult.body.code === 0 &&
      checkResult.body.data?.country === 'Test' &&
      engine.profiles.get('mcp-fp')?.exitNetwork?.country === 'Test'
  );
  ...
  const syncStart = await request('POST', '/api/sync/start', {
    profile_ids: ['mcp-fp', 'fp1'],
    operate: 'click,scroll',
  });
  record(
    'local-api sync start',
    syncStart.status === 200 && syncStart.body.code === 0 && syncTileCount === 1 && syncState.active === true
  );
  ...
  const spawnMcp = (env, extraCalls = []) =>
    new Promise((resolve) => {
      const child = spawn(process.execPath, ['automation/mcp-server.js'], {
        cwd: process.cwd(),
        env: {
          ...process.env,
          OPENBROWSER_API_PORT: String(apiPort),
          OPENBROWSER_API_KEY: 'mcp-selftest-key',
          ...env,
        },
      ...
      const calls = [
        ...
        { id: 3, method: 'tools/call', params: { name: 'mcp_policy', arguments: {} } },
        {
          ...
          params: {
            name: 'create_profile',
            arguments: { profile_id: 'mcp-b', name: 'MCP B', proxy: 'Direct' },
          },
        },
        { id: 5, method: 'tools/call', params: { name: 'list_profiles', arguments: {} } },
        {
          id: 6,
          method: 'tools/call',
          params: { name: 'get_fingerprint', arguments: { profile_id: 'mcp-fp' } },
        },
        {
          id: 7,
          method: 'tools/call',
          params: {
            name: 'rpa_run_steps',
            arguments: { profile_id: 'fp1', steps: [{ type: 'wait', ms: 1 }] },
          },
        },
      ];
      let nextId = 8;
  ...
  const admin = await spawnMcp({ OPENBROWSER_MCP_MODE: 'admin' }, [
    {
      ...
      args: {
        profile_id: 'mcp-fp',
        fingerprint: { os: 'Windows 11', hardwareConcurrency: 16 },
  ...
  record(
    'mcp create_profile',
    admin.get(4)?.result && admin.get(4)?.result.isError === false && engine.profiles.has('mcp-b')
  );
  record('mcp list_profiles', admin.get(5)?.result?.isError === false);
  record('mcp get_fingerprint', admin.get(6)?.result?.isError === false);
  record(
    'mcp fingerprint_set',
    admin.get(8)?.result?.isError === false &&
      engine.profiles.get('mcp-fp')?.privacy?.fingerprint?.os === 'Windows 11' &&
      engine.profiles.get('mcp-fp')?.width === 1920
  );
  ...
  record(
    'mcp fingerprint_reset',
    adminFp.get(8)?.result?.isError === false &&
      Object.keys(engine.profiles.get('mcp-fp')?.privacy?.fingerprint || {}).length === 0
  );
  record(
    'mcp fingerprint_regenerate',
    adminFp.get(9)?.result?.isError === false &&
      engine.profiles.get('mcp-fp')?.privacy?.refreshFingerprintOnStart === true &&
      engine.profiles.get('mcp-fp')?.privacy?.stabilityMode === 'off'
  );

  const read = await spawnMcp({ OPENBROWSER_MCP_MODE: 'read' });
  const readTools = read.get(2)?.result?.tools || [];
  record(
    'mcp read mode hides manage tools',
    readTools.length > 0 && !readTools.some((tool) => tool.name === 'create_profile')
  );
  ...
  const blacklist = await spawnMcp({
    OPENBROWSER_MCP_MODE: 'admin',
    OPENBROWSER_MCP_TOOL_BLACKLIST: JSON.stringify(['create_profile', 'rpa_run_steps']),
  });
  record(
    'mcp blacklist hides tools',
    !(blacklist.get(2)?.result?.tools || []).some((tool) => tool.name === 'create_profile')
  );
  ...
  const whitelist = await spawnMcp({
    OPENBROWSER_MCP_MODE: 'admin',
    OPENBROWSER_MCP_TOOL_WHITELIST: JSON.stringify(['mcp_policy', 'list_profiles']),
  });
  record(
    'mcp whitelist restricts tools',
    (whitelist.get(2)?.result?.tools || [])
      .map((tool) => tool.name)
      .sort()
      .join(',') === 'list_profiles,mcp_policy'
  );
  ...
  if (fail.length) {
    console.error(
      JSON.stringify(
        { success: false, checks: checks.length, failed: fail.map((item) => item.name) },
        null,
        2
      )
    );
