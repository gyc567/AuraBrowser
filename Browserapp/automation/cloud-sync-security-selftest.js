...
async function main() {
  ...
  assert.ok(
    cloud
      .webDavTarget({ url: 'http://127.0.0.1:9000/dav', dir: 'OpenBrowser' })
      .target.startsWith('http://127.0.0.1:9000/')
  );
  console.log('  PASS  WebDAV requires HTTPS except loopback');

  assert.throws(() =>
    cloud.validatedGitHubConfig({ owner: '../x', repo: 'repo', token: 't' }, cloud.REMOTE_NAME)
  );
  assert.throws(() =>
    cloud.validatedGitHubConfig(
      { owner: 'owner', repo: 'repo', token: 't', branch: '../main' },
      cloud.REMOTE_NAME
    )
  );
  ...
  try {
    ...
    if (process.platform !== 'win32') {
      ...
      await assert.rejects(() =>
        cloud.upload('local', { dir: root }, Buffer.from('x'), 'profiles/env.obpack')
      );
