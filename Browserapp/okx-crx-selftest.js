...
async function main() {
  const crxFile = 'C:/tmp/okx-wallet.crx';
  const root = path.join(__dirname, '..', 'okx-crx-selftest-data');
  await fs.rm(root, { recursive: true, force: true });
  try {
    const extension = await addChromeStoreExtension(
      ...
      JSON.stringify(
        {
          ...
          version: manifest.version,
          manifestVersion: manifest.manifest_version,
          files: (await fs.readdir(extension.path)).length,
        },
        null,
        2
      )
    ...
    await fs.rm(crxFile, { force: true }).catch(() => {});
  }
}
main().catch((error) => {
