...
ok(
  `select offers exactly persona/default (got ${optionValues.join(',')})`,
  JSON.stringify(optionValues) === JSON.stringify(['default', 'persona'])
);

// --- renderer read + write ---
ok(
  'renderer reads the select into the draft',
  /deviceProfile: \$\('#editor-device-profile'\)\?\.value \|\| 'default'/.test(renderer)
);
ok(
  'renderer populates the select from the profile',
  /editorSet\('#editor-device-profile', privacy\.deviceProfile \|\| 'default'\)/.test(renderer)
);
ok(
  'normalizing keeps an existing profile off by default',
  /deviceProfile: String\(privacy\.deviceProfile \|\| 'default'\)/.test(renderer)
);
...
// --- engine whitelist accepts exactly those values ---
{
  ...
  const of = (value) =>
    sanitize.call(ctx, { id: 'w', name: 'w', privacy: value === undefined ? {} : { deviceProfile: value } })
      .privacy.deviceProfile;
...
// --- the setting actually changes the fingerprint it is supposed to ---
{
  ...
  const off = buildFingerprint({
    id: 'wire',
    userAgent: WIN,
    privacy: { deviceProfile: 'default' },
    advanced: {},
  });
  const on = buildFingerprint({
    id: 'wire',
    userAgent: WIN,
  ...
  ok(
    'turning it on changes the reported hardware',
    JSON.stringify([on.hardwareConcurrency, on.deviceMemory, on.webgl.renderer]) !==
      JSON.stringify([off.hardwareConcurrency, off.deviceMemory, off.webgl.renderer])
  );
