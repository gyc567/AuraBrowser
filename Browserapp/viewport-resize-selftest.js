...
async function viewport(tab) {
  const result = await cdp.call(tab.webSocketDebuggerUrl, 'Runtime.evaluate', {
    expression:
      '({innerWidth,innerHeight,visualWidth:visualViewport.width,visualHeight:visualViewport.height,clientWidth:document.documentElement.clientWidth,clientHeight:document.documentElement.clientHeight})',
    returnByValue: true,
  });
...
async function main() {
  ...
  try {
    ...
    const clampedByWorkArea =
      process.platform === 'win32' &&
      ((largeBounds.width || 0) < requestedLarge.width - 100 ||
        (largeBounds.height || 0) < requestedLarge.height - 100);
    if (clampedByWorkArea) {
      ...
      assert.ok(
        ...
        `browser window did not enlarge before work-area clamp: ${JSON.stringify({ smallBounds, largeBounds })}`
      );
      assert.ok(
        viewportWidthDelta > 0 && viewportHeightDelta > 0,
        `page viewport did not follow clamped resize: ${JSON.stringify({ small, large })}`
      );
    } else {
      assert.ok(
        widthDelta > 150,
        `browser window did not resize: ${JSON.stringify({ smallBounds, largeBounds })}`
      );
      assert.ok(
        viewportWidthDelta > 150,
        `innerWidth did not follow resize: ${JSON.stringify({ small, large })}`
      );
      assert.ok(
        viewportHeightDelta > 100,
        `innerHeight did not follow resize: ${JSON.stringify({ small, large })}`
      );
    }
    assert.ok(
      Math.abs(large.innerWidth - large.visualWidth) < 3,
      `visual viewport is fixed/cropped: ${JSON.stringify(large)}`
    );
    assert.ok(
      Math.abs(large.innerWidth - large.clientWidth) < 3,
      `document viewport does not fill window: ${JSON.stringify(large)}`
    );

    process.stdout.write(
      JSON.stringify({ success: true, clampedByWorkArea, smallBounds, largeBounds, small, large }, null, 2)
    );
  } finally {
    await engine.stopAll().catch(() => {});
    await getStartPageServer()
      ?.stop?.()
      .catch(() => {});
