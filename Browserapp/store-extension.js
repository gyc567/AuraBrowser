...
function extractStoreId(value) {
  ...
  try {
    ...
    throw new Error('请输入有效的 Chrome 应用商店 URL');
  }
  if (!/(^|\.)chromewebstore\.google\.com$/i.test(parsed.hostname))
    throw new Error('只支持 Chrome 应用商店链接');
...
function download(url, redirects = 0) {
  ...
  if (parsed.protocol !== 'https:' || !allowed.includes(parsed.hostname.toLowerCase()))
    return Promise.reject(new Error('Chrome 商店返回了不受信任的下载地址'));
  return new Promise((resolve, reject) => {
    const request = https.get(
      parsed,
      { headers: { 'user-agent': 'Mozilla/5.0 OpenBrowserLocal/3.0' }, timeout: 25000 },
      (response) => {
        ...
        if (response.statusCode !== 200) {
          response.resume();
          return reject(new Error(`Chrome 商店下载失败（HTTP ${response.statusCode}）`));
        }
        const chunks = [];
        let size = 0;
        response.on('data', (chunk) => {
          size += chunk.length;
          if (size > 120 * 1024 * 1024) request.destroy(new Error('扩展包超过 120 MB 限制'));
...
function readVarint(buffer, state) {
  let value = 0;
  let shift = 0;
  while (state.offset < buffer.length && shift < 35) {
...
function protobufFields(buffer) {
  ...
  while (state.offset < buffer.length) {
    ...
    if (wire === 2) {
      ...
      if (end > buffer.length) throw new Error('CRX3 protobuf field is truncated');
      fields.push({ field, value: buffer.subarray(state.offset, end) });
      state.offset = end;
...
function extensionIdFromKey(publicKey) {
  const bytes = crypto.createHash('sha256').update(publicKey).digest().subarray(0, 16);
  let result = '';
...
function crxDetails(buffer) {
  if (buffer.length < 16 || buffer.subarray(0, 4).toString('ascii') !== 'Cr24')
    throw new Error('下载内容不是有效的 CRX 文件');
  ...
  if (version === 2) {
    ...
    const crxId = signedHeader
      ? protobufFields(signedHeader.value).find((item) => item.field === 1)?.value
      : null;
    if (!crxId || crxId.length !== 16)
      throw new Error('CRX3 signed header does not contain a valid extension id');
    extensionId = extensionIdFromBytes(crxId);
    const proofKeys = fields
      .filter((item) => item.field === 2 || item.field === 3)
  ...
  if (
    ...
    buffer.subarray(offset, offset + 2).toString('ascii') !== 'PK'
  )
    throw new Error('CRX 内没有有效 ZIP 数据');
  return { zip: buffer.subarray(offset), publicKey, extensionId };
}

function crxZip(buffer) {
  return crxDetails(buffer).zip;
}

function tarCommand() {
  if (process.platform === 'win32') {
    const systemRoot = process.env.SystemRoot || process.env.WINDIR || 'C:\\Windows';
    const candidates = [path.join(systemRoot, 'System32', 'tar.exe'), 'tar.exe', 'tar'];
...
function run(command, args, capture = false) {
  return new Promise((resolve, reject) => {
    ...
    child.once('exit', (code) =>
      code === 0 ? resolve(output) : reject(new Error(error.trim() || `${command} 执行失败（${code}）`))
    );
  });
}
...
async function addChromeStoreExtension(input, root, readManifest, fetchPackage = download) {
  ...
  const query = new URLSearchParams({
    ...
    x: `id=${storeId}&installsource=ondemand&uc`,
  });
  const crx = await fetchPackage(`https://clients2.google.com/service/update2/crx?${query}`);
  const details = crxDetails(crx);
  if (details.extensionId && details.extensionId !== storeId)
    throw new Error(`CRX 签名 ID ${details.extensionId} 与商店 ID ${storeId} 不一致`);
  ...
  try {
    ...
    if (details.publicKey) {
      ...
      if (!manifest.key) {
        manifest.key = details.publicKey.toString('base64');
