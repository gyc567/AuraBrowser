...
ok(
  /['"]retro-desktop['"]\s*:\s*\{\s*nameKey:\s*['"]theme\.retro\.name['"]/.test(renderer),
  'renderer registers retro-desktop'
);
...
ok(
  css.includes('--ui-radius: 10px') ||
    css.includes('border-radius: 10px') ||
    css.includes('border-radius: 12px'),
  'rounded radii present'
);
ok(
  css.includes('border-radius: 999px') || css.includes('border-radius: 12px'),
  'pill/round controls present'
);
...
ok(
  !css.includes('background-size: 32px 32px') && !css.includes('background-size: 16px 16px'),
  'no pixel grid'
);
...
ok(
  /html\[data-ui-theme="pixel-workstation"\] body \{[\s\S]*?color:\s*#e9f1eb/.test(pixel) ||
    pixel.includes('color: #e9f1eb'),
  'pixel body text bright'
);
ok(
  pixel.includes('--ui-text: #11181f') || pixel.includes('--theme-text: #11181f'),
  'pixel table/surface text is dark ink'
);
...
ok(
  fontCssText.includes("font-family: 'Ark Pixel'") && fontCssText.includes('ArkPixel12-zh_cn.woff2'),
  'font CSS declares Ark Pixel'
);
...
ok(
  !/font-family:\s*"Ark Pixel"[^;]*monospace\s*!important/.test(pixel),
  'pixel sidebar not mono-only after Ark Pixel'
);
ok(
  !/font-family:\s*"Ark Pixel"[^;]*monospace\s*!important/.test(nes),
  'nes sidebar not mono-only after Ark Pixel'
);
ok(
  /\.sidebar \.nav \{[\s\S]*?font-size:\s*16px !important/.test(pixel) ||
    pixel.includes('font-size: 16px !important'),
  'pixel sidebar nav larger'
);
ok(
  /\.sidebar \.nav \{[\s\S]*?font-size:\s*16px !important/.test(nes) ||
    nes.includes('font-size: 16px !important'),
  'nes sidebar nav larger'
);
