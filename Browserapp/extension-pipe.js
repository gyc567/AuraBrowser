...
class PipeConnection {
  ...
  onData(chunk) {
    ...
    for (;;) {
      ...
      clearTimeout(item.timer);
      if (value.error)
        item.reject(new Error(value.error.message || 'Chrome extension installer command failed'));
  ...
  command(method, params = {}, timeout = 15000) {
    ...
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new Error(`Chrome extension installer timeout: ${method}`));
      }, timeout);
      this.pending.set(id, { resolve, reject, timer });
      this.input.write(
        Buffer.concat([Buffer.from(JSON.stringify({ id, method, params }), 'utf8'), Buffer.from([0])]),
        (error) => {
          if (error) {
            clearTimeout(timer);
            this.pending.delete(id);
...
async function reconcileOnConnection(connection, desired, managedPaths = []) {
  ...
  for (const extension of current.extensions || []) {
    ...
    if (managed.includes(currentPath) && !wanted.has(currentPath))
      await connection.command('Extensions.uninstall', { id: extension.id });
  }
  const refreshed = await connection.command('Extensions.getExtensions');
  const present = new Map(
    (refreshed.extensions || []).map((item) => [String(item.path || '').toLowerCase(), item])
  );
  const installed = [];
  for (const extension of desired) {
    const existing = present.get(extension.path.toLowerCase());
    if (existing?.enabled) {
      installed.push({ ...extension, chromeExtensionId: existing.id });
      continue;
    }
    if (existing) await connection.command('Extensions.uninstall', { id: existing.id });
    const result = await connection.command(
      'Extensions.loadUnpacked',
      { path: extension.path, enableInIncognito: false },
      30000
    );
...
async function reconcileUnpackedExtensions(browserPath, root, desired, managedPaths = []) {
  ...
  try {
    return await reconcileOnConnection(installer.connection, desired, managedPaths);
  } finally {
    await closeInstaller(installer);
  }
}
...
module.exports = {
  ...
  launchInstaller,
  closeInstaller,
  reconcileOnConnection,
  reconcileUnpackedExtensions,
  portConnection,
};
