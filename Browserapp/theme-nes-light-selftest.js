...
ok(
  /['"]nes-light['"]\s*:\s*\{\s*nameKey:\s*['"]theme\.nes\.name['"]/.test(renderer),
  'renderer UI_THEMES registers nes-light'
);
ok(
  /colorScheme:\s*['"]light['"]/.test(renderer.match(/'nes-light':[\s\S]*?\}/)?.[0] || ''),
  'nes-light colorScheme is light'
);
...
ok(
  main.includes("'nes-light': { bg: '#c2b59c', overlay: '#d0c4aa', symbol: '#27231b' }"),
  'main THEME_CHROME has nes-light kraft chrome'
);
...
ok(
  i18n.includes("'theme.nes'") && i18n.includes("'theme.nes.name'") && i18n.includes("'theme.nes.desc'"),
  'i18n has zh/en theme.nes keys'
);
...
ok(
  css.includes('#cdc1a9') && css.includes('#51787a') && css.includes('#c2b59c') && css.includes('#e7ddc8'),
  'kraft paper palette present'
);
ok(
  !css.includes('#209cee') &&
    !css.includes('#c381b5') &&
...
ok(
  !/titlebar-integrated[\s\S]*?background:\s*#212529 !important/.test(css),
  'integrated titlebar no longer forces dark sidebar'
);
ok(css.includes('assets/logo-pixel.png'), 'brand logo uses assets/logo-pixel.png');
ok(
  /\.rpa-log[\s\S]*?background:\s*\#e7ddc8/.test(css) &&
    !/html\[data-ui-theme="nes-light"\] \.rpa-log \{[\s\S]*?#25221e/.test(css),
  'RPA run log kraft paper not dark island'
);
...
ok(
  css.includes('height: auto !important') && css.includes('min(68vw, 820px)'),
  'nes header auto height for long subtitles'
);
ok(
  pixel.includes('height: auto !important') && pixel.includes('min(68vw, 820px)'),
  'pixel header auto height for long subtitles'
);
ok(
  !/\[data-platform="macos"\] \.content > header \{ height: 52px/.test(css),
  'nes mac header no longer fixed 52px'
);
ok(
  !/\[data-platform="macos"\] \.content > header \{ height: 52px/.test(pixel),
  'pixel mac header no longer fixed 52px'
);
...
ok(
  !/nes-light\.css/.test(pack) || !/excluded/.test(pack),
  'packager does not special-case exclude nes-light'
);
