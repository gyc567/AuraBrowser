...
(async () => {
  ...
  // Child exited before the port was ready → throws (never hangs to timeout).
  {
    ...
    let threw = false;
    try {
      await waitForPort.call(ctx, dir, 5000, { exitCode: 1, signalCode: null, _startupDiagnostic: {} });
    } catch (e) {
      threw = /exited before CDP/.test(e.message);
    }
  ...
  // spawn error surfaces immediately.
  {
    ...
    let threw = false;
    try {
      await waitForPort.call(ctx, dir, 5000, {
        exitCode: null,
        _startupDiagnostic: { spawnError: 'ENOENT' },
      });
    } catch (e) {
      threw = /could not start: ENOENT/.test(e.message);
    }
  ...
  // Never-appears → clean timeout error.
  {
    ...
    let threw = false;
    try {
      await waitForPort.call(ctx, dir, 300, null);
    } catch (e) {
      threw = /port was not ready/.test(e.message);
    }
  ...
  console.error('waitforport-selftest FAILED:', e);
  process.exit(1);
});
