...
async function main() {
  ...
  try {
    ...
    const extensionPaths = ['mcohilncbfahbmgdjkbpemcciiolgcge', 'cdmhpjjhnamicehbdojmlnnodfcgnehn'].map(
      (id) => path.join(extensionRoot, id)
    );
    const extensions = [];
    for (const extensionPath of extensionPaths) extensions.push(await engine.addExtension(extensionPath));
    const profile = {
      id: 'startup-extension-probe',
      number: 1,
      name: '1',
    ...
    const finalTargets = await targets(running.port);
    const finalPages = finalTargets
      .filter((item) => item.type === 'page')
      .map((item) => ({ title: item.title, url: item.url, id: item.id }));
    if (finalPages.length !== 1 || !/openbrowser-start\\.html/i.test(finalPages[0].url))
      throw new Error('Startup extension page was not suppressed: ' + JSON.stringify(finalPages));
    process.stdout.write(
      JSON.stringify(
        {
          success: true,
          extensions: extensions.map((item) => item.name),
          pageTargets: values.filter((item) => item.type === 'page'),
          finalPageTargets: finalPages,
          extensionTargets: values.filter((item) => /^(chrome|edge)-extension:/i.test(item.url)),
        },
        null,
        2
      )
