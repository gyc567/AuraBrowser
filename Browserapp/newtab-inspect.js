...
async function main() {
  const root = path.join(__dirname, '..', 'newtab-inspect-data');
  ...
  try {
    ...
    const profile = {
      id: 'newtab-inspect',
      name: 'New Tab Inspect',
      browser: 'Google Chrome',
      proxy: 'Direct',
    };
    engine.syncProfiles([profile]);
    const running = await engine.start(profile);
    const tab = await cdp.newTab(running.port, 'chrome://newtab');
    await cdp.activateTab(running.port, tab.id);
    await sleep(1600);
    ...
    const inspected = await cdp.call(tab.webSocketDebuggerUrl, 'Runtime.evaluate', {
      expression,
      returnByValue: true,
    });
    let binding = 'ok';
    try {
      await cdp.call(tab.webSocketDebuggerUrl, 'Runtime.addBinding', { name: 'openBrowserInspect' });
    } catch (error) {
      binding = error.message;
    }
    process.stdout.write(
      JSON.stringify({ success: true, result: inspected.result?.value, binding }, null, 2)
    );
  } finally {
