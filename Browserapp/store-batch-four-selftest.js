...
async function fetchViaSystemProxy(url) {
  ...
  await new Promise((resolve, reject) => {
    ...
    child.once('exit', (code) =>
      code === 0 ? resolve() : reject(new Error(error || `curl exited ${code}`))
    );
  });
  try {
...
async function main() {
  ...
  const profiles = Array.from({ length: 4 }, (_, index) => ({
    id: `store-env-${index + 1}`,
    name: index ? `Store Slave ${index}` : 'Store Master',
  ...
  try {
    ...
    if (extension.storeId !== 'mcohilncbfahbmgdjkbpemcciiolgcge')
      throw new Error('unexpected store extension id');
    ...
    if (!enabledCard?.enabledAll || enabledCard.assignedProfiles !== 4)
      throw new Error('Application Center did not report the extension enabled for all four profiles');
    ...
    const enabledMatches = enabledLists.map((list) =>
      list.find(
        (item) => path.resolve(item.path).toLowerCase() === path.resolve(extension.path).toLowerCase()
      )
    );
    if (enabledMatches.some((item) => !item || !item.enabled))
      throw new Error(
        'OKX extension was not enabled in all four Google Chrome profiles: ' + JSON.stringify(enabledLists)
      );
    ...
    if (disabledCard?.enabledAll || disabledCard?.assignedProfiles !== 0)
      throw new Error('Application Center did not report the extension disabled for all profiles');
    ...
    if (
      disabledLists.some((list) =>
        list.some(
          (item) => path.resolve(item.path).toLowerCase() === path.resolve(extension.path).toLowerCase()
        )
      )
    )
      throw new Error('OKX extension remained after global disable');
      ...
      JSON.stringify(
        {
          ...
          applicationCenter: {
            enabledAll: enabledCard.enabledAll,
            enabledAssignments: enabledCard.assignedProfiles,
            disabledAll: disabledCard.enabledAll,
            disabledAssignments: disabledCard.assignedProfiles,
          },
          enabled: enabledMatches,
          disabledCounts: disabledLists.map((list) => list.length),
          browsers: enabledSessions.map((item) => item.browser),
        },
        null,
        2
      )
