...
async function main() {
  ...
  const profiles = Array.from({ length: 4 }, (_unused, index) => ({
  ...
  try {
    ...
    for (const item of running) {
      ...
      await cdp.activateTab(item.port, tab.id);
      tabs.push(tab);
    }
    await sleep(1800);
    for (const tab of tabs)
      if (!(await evaluate(tab, focusScript))) throw new Error('Focused input not found for ' + tab.id);
    const writes = [];
    for (let index = 0; index < running.length; index += 1)
      writes.push(await cdp.insertText(running[index].port, String(index + 1)));
    const values = [];
    for (const tab of tabs) values.push(await evaluate(tab, readScript));
    const expected = ['1', '2', '3', '4'];
    if (JSON.stringify(values) !== JSON.stringify(expected))
      throw new Error('Specified text mismatch: ' + JSON.stringify({ expected, values }));
    process.stdout.write(
      JSON.stringify(
        { success: true, expected, values, verifiedTargets: writes.map((item) => item.targetId) },
        null,
        2
      )
    );
