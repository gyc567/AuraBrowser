...
function run(file, args) {
  return new Promise((resolve, reject) => {
    ...
    child.once('exit', (code) =>
      code === 0 ? resolve(out.trim()) : reject(new Error(err || out || `devtools driver exited ${code}`))
    );
  });
}
...
async function devToolsTextState(target, text) {
  ...
  const result = await cdp.call(target.webSocketDebuggerUrl, 'Runtime.evaluate', {
    expression,
    returnByValue: true,
  });
...
async function main() {
  ...
  try {
    ...
    await wait(2400);
    if (!controller.nativeDevToolsMode)
      throw new Error(
        'native DevTools mode was not activated after master F12: ' + JSON.stringify(events.slice(-10))
      );
    const targetSnapshots = await Promise.all(sessions.map((session) => cdp.targets(session.port)));
    const devToolsTargets = targetSnapshots.map((items) =>
      items.find((item) => String(item.url || '').startsWith('devtools://'))
    );
    if (devToolsTargets.some((item) => !item))
      throw new Error('DevTools did not open in all four environments: ' + JSON.stringify(targetSnapshots));
    const sourceRects = await Promise.all(
      devToolsTargets.map((target) => devToolsTextState(target, 'Sources'))
    );
    if (sourceRects.some((item) => item.missing || !item.visible))
      throw new Error('Sources tab was not visible in all DevTools targets: ' + JSON.stringify(sourceRects));
    const source = sourceRects[0];
    const sourcePoint = [
      Math.round(source.screenX + source.x + source.width / 2),
      Math.round(source.screenY + (source.outerHeight - source.innerHeight) + source.y + source.height / 2),
    ];
    ...
    const moreRects = await Promise.all(
      devToolsTargets.map((target) => devToolsTextState(target, 'Show more'))
    );
    if (moreRects.some((item) => item.missing || !item.visible))
      throw new Error(
        'Sources Show more control was not visible in all environments: ' + JSON.stringify(moreRects)
      );
    const more = moreRects[0];
    const morePoint = [
      Math.round(more.screenX + more.x + more.width / 2),
      Math.round(more.screenY + (more.outerHeight - more.innerHeight) + more.y + more.height / 2),
    ];
    const moreClick = await run(driver, ['--coordinates', String(morePoint[0]), String(morePoint[1])]);
    await wait(500);
    const snippetRects = await Promise.all(
      devToolsTargets.map((target) => devToolsTextState(target, 'Snippets'))
    );
    let snippetPoint = null;
    let snippetsClick = 'not exposed by this Chromium build';
    let emptyStates = [];
    ...
    if (snippetsAvailable) {
      ...
      snippetPoint = [
        Math.round(snippet.screenX + snippet.x + snippet.width / 2),
        Math.round(
          snippet.screenY + (snippet.outerHeight - snippet.innerHeight) + snippet.y + snippet.height / 2
        ),
      ];
      snippetsClick = await run(driver, ['--coordinates', String(snippetPoint[0]), String(snippetPoint[1])]);
      await wait(900);
      emptyStates = await Promise.all(
        devToolsTargets.map((target) => devToolsTextState(target, 'No snippets saved'))
      );
      if (emptyStates.some((item) => item.missing || !item.visible))
        throw new Error(
          'Snippets empty state did not synchronize in all environments: ' + JSON.stringify(emptyStates)
        );
    }
    process.stdout.write(
      JSON.stringify(
        {
          success: true,
          fourChrome: sessions.map((session) => ({ id: session.id, pid: session.pid })),
          f12Output,
          devToolsTargets: devToolsTargets.length,
          sourcePoint,
          sourcesClick,
          sourceStates,
          morePoint,
          ...
          snippetsClick,
          emptyStates,
          nativeDevToolsMode: controller.nativeDevToolsMode,
        },
        null,
        2
      )
