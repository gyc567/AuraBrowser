...
function main() {
  ...
  const kernelPath = path.join(
    __dirname,
    'kernels/macos-x64/chrome_148/openbrowser_148/OpenBrowser.app/Contents/MacOS/OpenBrowser'
  );
  const dockCmd =
    '/Users/x/Library/Application Support/openbrowser/env-apps/id/环境 1.app/Contents/MacOS/OpenBrowser.bin --user-data-dir=/tmp/p --remote-debugging-port=0';
  const kernelCmd = `${kernelPath} --user-data-dir=/tmp/p`;

  assert.strictEqual(
    commandMatchesExecutable(dockCmd, kernelPath),
    true,
    'kernel OpenBrowser basename should match OpenBrowser.bin dock process'
  );
  ...
  assert.strictEqual(
    commandMatchesExecutable(dockCmd, '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'),
    false
  );
  ...
  if (process.platform !== 'win32') {
    assert.strictEqual(
      fake.ok,
      true,
      'self node process should match expectedExecutables, got ' + JSON.stringify(fake)
    );
  }
  ...
  assert.ok(
    !/copyFile\(src, dest\);\s*await fsp\.chmod\(dest, 0o755\);\s*\}\s*catch \(_\) \{\s*await forceSymlink\(src, dest\);/s.test(
      envIcon
    ),
    'must not symlink OpenBrowser.bin on copy failure'
  );
  ...
  assert.ok(
    !/killProcessTree\([^)]*expectedExecutable:\s*(item\.browser|browser)\.path/.test(engineSrc),
    'must not pass only browser.path as expectedExecutable'
  );
  assert.ok(engineSrc.includes('isOpenBrowser148(browser)'), 'Dock shell only for openbrowser-148');
  assert.ok(
    engineSrc.includes('ipc-stub\\\\.py') || engineSrc.includes('ipc-stub\\.py'),
    'pkill pattern must be anchored'
  );
  ...
  assert.ok(
    envIcon.includes('ipc-stub\\\\.py') || envIcon.includes('ipc-stub\\.py'),
    'launcher pkill must be anchored'
  );
