...
async function run() {
  ...
  engine.extensions.set('ext-global', {
    id: 'ext-global',
    name: 'Global Test',
    version: '1.0.0',
    path: root,
  ...
  for (const profile of added)
    assert(
      engine.assignments.get(profile.id).has('ext-global'),
      profile.id + ' did not inherit the global extension'
    );
  ...
  assert(
    fs.existsSync(engine.profileRoot('env-007')),
    'Browser data should be retained when deleteData is false'
  );
  assert.strictEqual(engine.status().length, 4);
  await fsp.rm(root, { recursive: true, force: true });
  console.log(
    'PROFILE_BATCH_SELFTEST_OK created=3 deleted=3 stopped=1 data_delete=2 data_retain=1 extension_inheritance=3'
  );
}
