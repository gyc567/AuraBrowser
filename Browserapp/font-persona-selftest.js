...
{
  ok(
    'Windows set has Segoe UI and Calibri',
    fontsForOs('windows').includes('Segoe UI') && fontsForOs('windows').includes('Calibri')
  );
  ok('Windows set has no Helvetica Neue', !fontsForOs('windows').includes('Helvetica Neue'));
  ok(
    'macOS set has Helvetica Neue and Menlo',
    fontsForOs('macos').includes('Helvetica Neue') && fontsForOs('macos').includes('Menlo')
  );
  ok('macOS set has no Segoe UI', !fontsForOs('macos').includes('Segoe UI'));
  ok(
    'Linux set is DejaVu/Liberation based',
    fontsForOs('linux').includes('DejaVu Sans') && fontsForOs('linux').includes('Liberation Sans')
  );
  ...
  ok(
    'a platform never lists its own font as foreign',
    OS_FONTS.windows.every((f) => !winForeign.includes(f.toLowerCase()))
  );
}
...
// matching. That is invisible in the source and only shows up as a hook that never fires.
{
  ...
  const suspicious = literals.filter(
    (r) => /\((\?:)?\^\|s\)/.test(r) || /\)s\*/.test(r) || /[^\\]\bs\+/.test(r) || /[^\\]\bd\+/.test(r)
  );
...
// --- fingerprint output ---
{
  const persona = buildFingerprint({
    id: 'font-a',
    userAgent: WIN_UA,
    privacy: { deviceProfile: 'persona' },
    advanced: {},
  ...
  ok(
    'persona font set matches the claimed OS',
    persona.fonts.os === 'windows' && persona.fonts.list.includes('Segoe UI')
  );
...
function makeCtx(fp, opts = {}) {
  ...
  const win = {
    ...
    queryLocalFonts: async () =>
      (opts.hostFonts || ['Helvetica Neue', 'SF Pro']).map((family) => ({
        family,
        fullName: family,
...
// --- Local Font Access reports the persona, not the host ---
{
  ...
  return families.then((list) => {
    ...
    ok(
      'check() ignores a numeric font-weight before the size',
      check(ctx, 'italic 700 16px Segoe UI') === true
    );
    ok(
      'check() reads the primary family of a list',
      check(ctx, 'bold 12px/1.4 "Helvetica Neue", Arial') === false
    );

    const macFp = buildFingerprint({
      id: 'font-c',
      userAgent: MAC_UA,
      privacy: { deviceProfile: 'persona' },
      advanced: {},
