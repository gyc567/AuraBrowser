...
function parseProxy(value) {
  ...
  if (legacyScheme) {
    protocol = legacyScheme[1].toLowerCase();
    host = legacyScheme[2];
    port = Number(legacyScheme[3]);
    username = legacyScheme[4];
    password = legacyScheme[5];
    ...
    port = Number(parsed.port || (protocol === 'https' ? 443 : 0));
  ...
  if ((username || password) && protocol === 'socks4')
    throw new Error('Authenticated SOCKS proxies must use SOCKS5');
...
function resolveTlsProfile(input = {}) {
  ...
  let id = String(raw.id || raw.profile || raw.name || '')
    .trim()
    .toLowerCase();
  if (!id || id === 'auto') {
  ...
  const profile = {
    ...
    alpn:
      Array.isArray(raw.alpn) && raw.alpn.length
        ? raw.alpn
            .map((v) => String(v))
            .filter(Boolean)
            .slice(0, 8)
    ...
    permuteExtensions:
      raw.permuteExtensions != null ? Boolean(raw.permuteExtensions) : base.permuteExtensions,
...
class BufferedReader {
  ...
  read(size, timeout = 15000) {
    return this.wait({ type: 'size', size, timeout });
  }
  readUntil(marker, max = 65536, timeout = 15000) {
    return this.wait({ type: 'marker', marker: Buffer.from(marker), max, timeout });
  }
  wait(options) {
  ...
  pump() {
    ...
    if (waiter.type === 'marker') {
      ...
      else if (this.buffer.length > waiter.max)
        return this.fail(new Error('Proxy response header was too large'));
    }
    if (end < 0) return;
    this.waiters.shift();
...
async function readSocksAddress(reader, atyp) {
  ...
  if (atyp === 3) {
    ...
    return Buffer.concat([size, await reader.read(size[0])]);
  }
...
function encodeSocksAddress(host) {
  ...
  if (!value.length || value.length > 255) throw new Error('SOCKS5 target hostname is invalid');
...
async function connectSocksTargetOnce(config, host, port, signal = null) {
  let upstream;
  try {
    upstream = await connectSocket(config.host, config.port, 8000, signal);
    upstream.on('error', () => {});
    const reader = new BufferedReader(upstream);
    const timeout = 8000;
    upstream.write(config.authenticated ? Buffer.from([5, 2, 0, 2]) : Buffer.from([5, 1, 0]));
    const method = await reader.read(2, timeout);
    if (method[0] !== 5 || method[1] === 255)
      throw new Error('SOCKS5 proxy rejected available authentication methods');
    if (method[1] === 2) {
      const user = Buffer.from(config.username, 'utf8');
      const password = Buffer.from(config.password, 'utf8');
      if (!user.length || user.length > 255 || !password.length || password.length > 255)
        throw new Error('SOCKS5 username or password length is invalid');
      upstream.write(
        Buffer.concat([Buffer.from([1, user.length]), user, Buffer.from([password.length]), password])
      );
      const auth = await reader.read(2, timeout);
      if (auth[1] !== 0) throw new Error('SOCKS5 authentication failed');
    } else if (method[1] !== 0) throw new Error('SOCKS5 proxy selected an unsupported authentication method');
    upstream.write(
      Buffer.concat([Buffer.from([5, 1, 0]), encodeSocksAddress(host), Buffer.from([port >> 8, port & 255])])
    );
    const response = await reader.read(4, timeout);
    await readSocksAddress(reader, response[3]);
    await reader.read(2, timeout);
...
function retryableSocksError(error) {
  ...
  return !/authentication failed|rejected available authentication|unsupported authentication|username or password length|target hostname is invalid|upstream connection failed with code/i.test(
    message
  );
}
...
async function connectSocksTarget(config, host, port, attempts = 3, signal = null) {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    ...
    try {
      return await connectSocksTargetOnce(config, host, port, signal);
    } catch (error) {
      lastError = error;
...
function parseHttpTarget(header) {
  ...
  if (method === 'CONNECT') {
    ...
    if (separator < 1) throw new Error('Invalid HTTP CONNECT target');
    return {
      method,
      host: target.slice(0, separator).replace(/^\[|\]$/g, ''),
  ...
  return {
    ...
    header: lines
      .filter((line) => !/^proxy-authorization:/i.test(line) && !/^proxy-connection:/i.test(line))
      .join('\r\n'),
  };
}
...
async function startHttpToSocks5Bridge(config, onStatus) {
  ...
  const notify = makeNotifier(onStatus);
  ...
  const connectTarget = concurrentConnector(
    (host, port) => connectSocksTarget(config, host, port, 3, controller.signal),
    4,
    60
  );
  const server = net.createServer((client) => {
    ...
    const receiveHeader = (chunk) => {
      ...
      client.pause();
      const header = pending.subarray(0, marker + 4).toString('latin1');
      const body = pending.subarray(marker + 4);
      let target = null;
      (async () => {
        target = parseHttpTarget(header);
        if (process.env.OPENBROWSER_PROXY_DIAGNOSTICS === '1')
          notify('REQUEST', target.host + ':' + target.port);
        if (
          /^(mtalk\.google\.com|android\.clients\.google\.com|connectivitycheck\.gstatic\.com|update\.googleapis\.com)$/i.test(
            target.host
          )
        ) {
          client.end(
            target.method === 'CONNECT'
              ? 'HTTP/1.1 502 Background Request Blocked\r\nConnection: close\r\n\r\n'
              : 'HTTP/1.1 204 No Content\r\nConnection: close\r\n\r\n'
          );
          return;
        }
        const connected = await connectTarget(target.host, target.port);
        const upstream = connected.upstream;
        if (process.env.OPENBROWSER_PROXY_DIAGNOSTICS === '1')
          notify('CONNECTED', target.host + ':' + target.port);
        if (client.destroyed) {
          upstream.destroy();
          return;
        }
        ...
        if (connected.remainder.length) client.write(connected.remainder);
        ...
        notify(
          error.message.includes('authentication') ? 'AUTH_FAILED' : 'UPSTREAM_CONNECT_FAILED',
          (target?.host ? target.host + ':' + target.port + ' ? ' : '') + error.message
        );
...
async function startSocks5Bridge(config, onStatus) {
  ...
  const server = net.createServer((client) => {
    ...
    (async () => {
      ...
      const greeting = await local.read(2);
      if (greeting[0] !== 5) throw new Error('Invalid local SOCKS5 greeting');
      await local.read(greeting[1]);
      client.write(Buffer.from([5, 0]));
      const requestHead = await local.read(4);
      if (requestHead[0] !== 5 || requestHead[1] !== 1) throw new Error('Only SOCKS5 CONNECT is supported');
      const requestAddress = await readSocksAddress(local, requestHead[3]);
      const requestPort = await local.read(2);
      const upstream = await connectSocket(config.host, config.port);
      sockets.add(upstream);
      upstream.on('error', () => {
      ...
      upstream.write(config.authenticated ? Buffer.from([5, 2, 0, 2]) : Buffer.from([5, 1, 0]));
      const method = await remote.read(2);
      if (method[0] !== 5 || method[1] === 255)
        throw new Error('SOCKS5 proxy rejected available authentication methods');
      if (method[1] === 2) {
        const user = Buffer.from(config.username, 'utf8');
        ...
        if (auth[1] !== 0) {
          notify('AUTH_FAILED', 'SOCKS5 username or password was rejected');
      ...
      const responseAddress = await readSocksAddress(remote, responseHead[3]);
      const responsePort = await remote.read(2);
      ...
      if (responseHead[1] !== 0) {
        ...
        throw new Error('SOCKS5 upstream connection failed');
      }
      const localRemainder = local.release();
      const remoteRemainder = remote.release();
      if (localRemainder.length) upstream.write(localRemainder);
      if (remoteRemainder.length) client.write(remoteRemainder);
      client.pipe(upstream);
      upstream.pipe(client);
    })().catch((error) => {
      notify(
...
function forwardedHeader(header, config) {
  ...
  const kept = lines.filter(
    (line) => line && !/^proxy-authorization:/i.test(line) && !/^proxy-connection:/i.test(line)
  );
...
async function startHttpBridge(config, onStatus) {
  ...
  const server = net.createServer((client) => {
    ...
    const receiveHeader = (chunk) => {
      ...
      const marker = pending.indexOf('\r\n\r\n');
      if (marker < 0) return;
      client.removeListener('data', receiveHeader);
      client.pause();
      const header = pending.subarray(0, marker + 4).toString('latin1');
      const remainder = pending.subarray(marker + 4);
      const isConnect = /^CONNECT\s+/i.test(header.split('\r\n', 1)[0]);
      let upstream;
      const fail = (message) => {
        notify('UPSTREAM_CONNECT_FAILED', message);
        if (!client.destroyed) client.end('HTTP/1.1 502 Bad Gateway\r\nConnection: close\r\n\r\n');
      };
      try {
        upstream = connectHttpUpstream(config, () => {
          ...
          const receiveResponse = (data) => {
            ...
            if (response.length > 65536) {
              upstream.destroy();
              fail('Proxy response header was too large');
              return;
            }
            const responseMarker = response.indexOf('\r\n\r\n');
            if (responseMarker < 0) return;
            upstream.removeListener('data', receiveResponse);
            const responseHeader = response.subarray(0, responseMarker + 4);
            const responseRemainder = response.subarray(responseMarker + 4);
            const match = responseHeader
              .toString('latin1')
              .split('\r\n', 1)[0]
              .match(/\s(\d{3})(?:\s|$)/);
            ...
            if (status !== 200) {
              notify(
                status === 407 ? 'AUTH_FAILED' : 'TUNNEL_FAILED',
                status === 407
                  ? 'Proxy username or password was rejected'
      ...
      upstream.once('error', (error) => {
        if (!client.destroyed) fail(error.message);
        client.destroy();
      });
...
async function connectBridge(bridge, hostname, port) {
  const socket = await connectSocket('127.0.0.1', bridge.port);
  ...
  if (bridge.protocol === 'http') {
    ...
    if (greeting[1] !== 0) throw new Error('Local SOCKS5 bridge rejected no-auth mode');
    const host = Buffer.from(hostname, 'utf8');
    socket.write(
      Buffer.concat([Buffer.from([5, 1, 0, 3, host.length]), host, Buffer.from([port >> 8, port & 255])])
    );
    const reply = await reader.read(4);
    await readSocksAddress(reader, reply[3]);
    await reader.read(2);
    if (reply[1] !== 0) throw new Error('SOCKS5 proxy test tunnel failed with code ' + reply[1]);
  }
  const remainder = reader.release();
  if (remainder.length) throw new Error('Unexpected bytes before TLS handshake');
  return socket;
}
...
function decodeChunked(buffer) {
  ...
  while (offset < buffer.length) {
    ...
    if (line < 0) throw new Error('Invalid chunked response');
    const size = Number.parseInt(buffer.subarray(offset, line).toString('ascii').split(';')[0], 16);
  ...
  try {
    socket =
      bridge.protocol === 'http'
        ? await connectSocket('127.0.0.1', bridge.port)
        : await connectBridge(bridge, hostname, 80);
    ...
    const chunks = await new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        socket.destroy();
        reject(new Error('Proxy exit lookup timed out'));
      }, 15000);
...
async function requestProxyHttps(config, hostname, pathname) {
  ...
  try {
    ...
    const response = await new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        secure.destroy();
        reject(new Error('HTTPS proxy request timed out'));
      }, 15000);
...
async function lookupIpPureProxy(config) {
  ...
  if (response.status !== 200)
    throw new Error('risk intelligence proxy lookup returned HTTP ' + response.status);
...
function mergeNetworkLookups(parts = []) {
  ...
  const countryNote = geoConflict
    ? `多源地区不一致：${countries.join(' / ')}${
        usageCountry && registryCountry && usageCountry !== registryCountry
...
function classifyProxyError(error) {
  ...
  if (/authentication failed|username or password|407|rejected available authentication/i.test(msg))
    return 'auth';
  if (/timed?\s*out|timeout/i.test(msg)) return 'timeout';
  if (/ECONNREFUSED|ENOTFOUND|EHOSTUNREACH|ENETUNREACH|socket hang up|connect/i.test(msg))
    return 'unreachable';
      ...
      (res) => {
        ...
        if (status >= 300 && status < 400 && res.headers.location) {
          ...
          try {
            ...
            fetchUrlText(next, { timeout, method, allowPrivate, maxRedirects: maxRedirects - 1 }).then(
              resolve,
              reject
            );
          } catch (error) {
  ...
  try {
    ...
    const walk = (node, depth = 0) => {
      ...
      if (typeof node === 'object') {
        ...
        const protocol = String(node.protocol || node.type || node.schema || 'http')
          .toLowerCase()
          .replace('://', '');
        if (host && port) {
...
function normalizeIpWhoResult(value) {
  ...
  return {
    ...
    zip: String(value.postal || value.zip || ''),
    timezone: tz,
    latitude: Number.isFinite(Number(value.latitude ?? value.lat))
      ? Number(value.latitude ?? value.lat)
      : null,
    longitude: Number.isFinite(Number(value.longitude ?? value.lon))
      ? Number(value.longitude ?? value.lon)
      : null,
    isp: String(value.connection?.isp || value.isp || ''),
    organization: String(value.connection?.org || value.org || ''),
    asn: String(value.connection?.asn || value.asn || value.as || '')
      .toString()
      .split(/\s+/, 1)[0],
...
async function lookupDirectIpApi() {
  const fields =
    'status,message,country,countryCode,regionName,city,zip,timezone,lat,lon,isp,org,as,asname,mobile,proxy,hosting,query';
...
async function lookupDirectCountry() {
  const parts = await raceSettledValues(
    [lookupDirectIpApi, lookupDirectIpWho, lookupDirectIpInfo, lookupDirectCloudflareTrace],
    { maxWaitMs: 11000 }
  );
...
async function probeProxyHttps(
  config,
  ...
  try {
    ...
    secure = tls.connect(
      tlsConnectOptionsFromProfile(tlsProfile, { socket, servername: hostname, rejectUnauthorized: true })
    );
    await new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        secure.destroy();
        reject(new Error('Google HTTPS handshake timed out'));
      }, 15000);
      secure.once('secureConnect', () => {
        clearTimeout(timer);
        resolve();
