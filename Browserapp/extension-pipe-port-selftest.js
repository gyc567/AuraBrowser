...
async function main() {
  const root = path.join(__dirname, '..', 'extension-pipe-port-selftest-data');
  ...
  try {
    const loaded = await installer.connection.command('Extensions.loadUnpacked', {
      path: path.join(__dirname, 'bundled-extension'),
      enableInIncognito: false,
    });
    const port = await waitForPort(root);
    process.stdout.write(
      JSON.stringify(
        { success: Boolean(port), port, extensionId: loaded.id, processId: installer.child.pid },
        null,
        2
      )
    );
