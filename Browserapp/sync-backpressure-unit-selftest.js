...
async function main() {
  ...
  controller.slaves = [{ id: 'slave' }];
  controller.forward = async (_tabId, payload) => {
    await sleep(3);
    observed.push(payload);
  };

  for (let index = 0; index < 1000; index += 1)
    controller.enqueueForward('tab-1', { type: 'input', selector: '#q', value: String(index) });
  ...
  if (controller.forwardStats.coalesced < 900)
    throw new Error('expected high-frequency events to be coalesced');
  if (!events.some((event) => event.type === 'sync-health'))
    throw new Error('sync health events were not emitted');

  process.stdout.write(
    JSON.stringify({
      success: true,
      receivedInputs: inputs.length,
      coalesced: controller.forwardStats.coalesced,
      queueDepth: controller.forwardQueue.length,
      healthEvents: events.filter((event) => event.type === 'sync-health').length,
    })
  );
}
