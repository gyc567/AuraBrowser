...
const tableHead =
  '<div class="table-card sync-table"><table><thead><tr><th><input type="checkbox" id="select-all-sessions"></th><th>&#29615;&#22659;&#32534;&#21495;</th><th>&#29615;&#22659;&#21517;&#31216;</th><th>&#27983;&#35272;&#22120;</th><th>&#26631;&#31614;&#39029;</th><th>&#29366;&#24577;</th><th>&#25805;&#20316;</th></tr></thead><tbody id="session-table"></tbody></table>';
html = html.replace(
  /<div class="table-card sync-table"><table><thead><tr>.*?<\/tr><\/thead><tbody id="session-table"><\/tbody><\/table>/,
  tableHead
);
...
renderer = renderer.replace(
  /function renderSessions\(\) \{[\s\S]*?(?=function renderTabInventory\(\))/,
  sessionFunctions
);
renderer = renderer.replace(
  /^  const showWindow = .*$/m,
  String.raw`  const showWindow = event.target.closest('[data-show-window]'); if (showWindow) runSyncAction('\u663e\u793a\u7a97\u53e3', () => window.ops.windowAction([showWindow.dataset.showWindow], 'normal'));`
);
renderer = renderer.replace(
  /^\$\('#refresh-sessions'\)\.addEventListener[\s\S]*?(?=^\$\('#send-text'\))/m,
  String.raw`$('#refresh-sessions').addEventListener('click', refreshSessions);
...
renderer = renderer.replace(
  /^window\.ops\.onEvent.*$/m,
  String.raw`window.ops.onEvent(async (value) => { if (value.type === 'status') { await refreshStatus(); await refreshSessions(); } if (value.type === 'extensions') await refreshExtensions(); if (value.type === 'sync-state') { syncState = { active: value.active, master: value.master, selected: value.selected || [] }; renderSessions(); log('Sync', value.active ? '\u540c\u6b65\u5df2\u542f\u52a8' : '\u540c\u6b65\u5df2\u505c\u6b62'); } if (value.type === 'sync-error') { toast(value.message); log('Error', value.message); } });`
);
fs.writeFileSync(path.join(root, 'renderer.js'), renderer, 'utf8');

let main = fs.readFileSync(path.join(root, 'main.js'), 'utf8');
main = main.replace(
  /if \(selected\.length < 2\) throw new Error\('.*?'\);/,
  String.raw`if (selected.length < 2) throw new Error('\u8bf7\u81f3\u5c11\u9009\u62e9\u4e24\u4e2a\u8fd0\u884c\u4e2d\u7684\u6d4f\u89c8\u5668\u73af\u5883');`
);
