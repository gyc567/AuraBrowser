...
async function main() {
  ...
  try {
    ...
    await engine.start(profiles[0]);
    await engine.start(profiles[1]);
    ...
    const refused = events.filter(
      (event) => event.type === 'sync-error' && /ECONNREFUSED|connection refused/i.test(String(event.message))
    );
    if (disconnected.length !== 1)
      throw new Error(`expected one disconnect event, got ${disconnected.length}: ${JSON.stringify(events)}`);
    if (refused.length !== 0) throw new Error(`raw connection errors leaked: ${JSON.stringify(refused)}`);
    if (sync.timer !== null || sync.master !== null || sync.nativeInputMirror !== null)
      throw new Error('sync resources were not stopped');
    process.stdout.write(
      JSON.stringify(
        {
          success: true,
          disconnected: disconnected.length,
          rawConnectionErrors: refused.length,
          timerStopped: sync.timer === null,
          inputBridgeStopped: sync.nativeInputMirror === null,
          message: disconnected[0].message,
        },
        null,
        2
      )
