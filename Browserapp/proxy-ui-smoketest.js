...
async function main() {
  ...
  const id = process.argv[3] || 'env-001';
  if (action === 'inspect') {
  ...
  if (action === 'result') {
    console.log(
      JSON.stringify(
        await evaluate(
          '(() => { const s = window.__proxySmoke || { status: "idle" }; return { status: s.status, ip: s.ip || "", country: s.country || "", message: s.message || "" }; })()'
        )
      )
    );
    return;
  ...
  if (action === 'test-endpoint') {
    ...
    if (!host || !Number.isInteger(port)) throw new Error('Host and port are required');
    console.log(
      JSON.stringify(
        await evaluate(`(() => {
      const ui = JSON.parse(localStorage.getItem('openbrowser-ui-state') || '{}'); const profile = (ui.profiles || []).find((item) => item.id === ${JSON.stringify(id)});
      ...
      JSON.stringify(
        profile
          ? {
              ...
              port: profile.port || 0,
              exitIp: profile.network?.ip || profile.exitIp || '',
              country: profile.network?.countryCode || profile.exitCountryCode || '',
            }
          : { status: 'missing' }
      )
    );
  ...
  if (action === 'google') {
    ...
    console.log(JSON.stringify({ id, port: profile.port, targetId: tab.id }));
    return;
  }
  if (action === 'google-result') {
    const values = await evaluate('(async () => await window.ops.profileStatus())()', true);
    const profile = (values || []).find((item) => item.id === id);
    const tab = (await cdp.tabs(profile?.port || 0)).find((item) =>
      item.url.startsWith('https://www.google.com')
    );
    if (!tab) throw new Error('Google tab was not found');
    const result = await cdp.call(tab.webSocketDebuggerUrl, 'Runtime.evaluate', {
      expression:
        '({ title: document.title, text: (document.body?.innerText || "").slice(0, 300), readyState: document.readyState })',
      returnByValue: true,
    });
    ...
    console.log(
      JSON.stringify({
        ...
        hasError: /ERR_|This site can.t be reached|\\u65e0\\u6cd5\\u8bbf\\u95ee\\u6b64\\u7f51\\u7ad9/i.test(
          value.text
        ),
        hasGoogle: /Google/i.test(value.text),
      })
    );
    return;
  }
  if (action === 'logs') {
    console.log(
      JSON.stringify(
        await evaluate(
          `(() => { const ui = JSON.parse(localStorage.getItem('openbrowser-ui-state') || '{}'); return (ui.logs || []).slice(0, 12); })()`
        )
      )
    );
    return;
  ...
  if (action === 'stop') {
    console.log(
      JSON.stringify(
        await evaluate(`(async () => await window.ops.stopProfile(${JSON.stringify(id)}))()`, true)
      )
    );
    return;
  }
  ...
  if (action === 'bulk-result') {
    console.log(JSON.stringify(await evaluate('window.__bulkProxy || { status: "idle", results: [] }')));
    return;
  }
