...
async function typeText(tab, focusExpression, text) {
  await evaluate(
    tab,
    `(() => { const e=${focusExpression}; e.focus(); if('value' in e){const p=e instanceof HTMLTextAreaElement?HTMLTextAreaElement.prototype:HTMLInputElement.prototype;Object.getOwnPropertyDescriptor(p,'value').set.call(e,'');}else e.textContent=''; e.dispatchEvent(new InputEvent('input',{bubbles:true,composed:true,inputType:'deleteContent'})); return true; })()`
  );
...
async function main() {
  ...
  const url = `http://127.0.0.1:${port}/matrix`;
  const root = path.join(__dirname, '..', 'four-window-input-matrix-data');
  ...
  const profiles = Array.from({ length: 4 }, (_, index) => ({
    id: `input-env-${index + 1}`,
    name: index ? `Input Slave ${index}` : 'Input Master',
  ...
  try {
    ...
    const masterTab = await cdp.newTab(sessions[0].port, url);
    sync = new LiveSyncController(engine, (event) => events.push(event));
    await sync.start(profiles.map((profile) => profile.id));
    await cdp.activateTab(sessions[0].port, masterTab.id);
    await sleep(2200);
    const cases = [
      {
        name: 'dynamic',
        focus: "document.querySelector('[data-role=dynamic]')",
        read: "document.querySelector('[data-role=dynamic]').value",
        text: 'DYNX',
        expected: 'DYN',
      },
      {
        name: 'controlled',
        focus: "document.querySelector('#controlled')",
        read: "({value:document.querySelector('#controlled').value,state:document.querySelector('#controlled').dataset.state})",
        text: 'CTRLX',
        expected: { value: 'CTRL', state: 'CTRL' },
      },
      {
        name: 'textarea',
        focus: "document.querySelector('#area')",
        read: "document.querySelector('#area').value",
        text: 'AREAX',
        expected: 'AREA',
      },
      {
        name: 'contenteditable',
        focus: "document.querySelector('#editable')",
        read: "document.querySelector('#editable').textContent",
        text: 'EDITX',
        expected: 'EDIT',
      },
      {
        name: 'shadow',
        focus: "document.querySelector('#shadow').shadowRoot.querySelector('input')",
        read: "document.querySelector('#shadow').shadowRoot.querySelector('input').value",
        text: 'SHADX',
        expected: 'SHAD',
      },
      {
        name: 'iframe',
        focus: "document.querySelector('#frame').contentDocument.querySelector('input')",
        read: "document.querySelector('#frame').contentDocument.querySelector('input').value",
        text: 'FRAMX',
        expected: 'FRAM',
      },
    ];
    ...
    for (const test of cases) {
      await typeText(masterTab, test.focus, test.text);
      const mapping = sync.tabMap.get(masterTab.id);
      const values = [await evaluate(masterTab, test.read)];
      for (const slave of sync.slaves) {
        const tab = (await cdp.tabs(slave.port)).find((item) => item.id === mapping.get(slave.id));
        values.push(await evaluate(tab, test.read));
      }
      const pass = values.every((value) => JSON.stringify(value) === JSON.stringify(test.expected));
      if (!pass) throw new Error(`${test.name} failed: ${JSON.stringify(values)}`);
      results.push({ name: test.name, values });
    }
      ...
      JSON.stringify(
        {
          ...
          cases: results,
          errors: 0,
          skippedOverlappingRefreshes: sync.skippedRefreshes,
        },
        null,
        2
      )
